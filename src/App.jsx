...
async function callAI(prompt, sys = "You are an expert GTM/RevOps consultant. Improve the given content for clarity, professionalism, and strategic impact. Return ONLY the improved text.") {
  try { const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: prompt }], system: sys }) }); const d = await r.json(); return d.content?.[0]?.text || "AI enhancement failed."; } catch { return "AI service unavailable."; }
}
...
const SEED = {
  ...
  portfolioSettings: {
    headline: "Revenue operations, architected with precision",
    subheadline: "I partner with B2B companies to build scalable GTM systems that turn complexity into predictable growth.",
    services: [
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--ink)" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "20px 64px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(11,11,11,0.88)", backdropFilter: "blur(16px)", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}><span style={{ fontFamily: "var(--serif)", fontSize: 24, fontStyle: "italic", color: "var(--cream)" }}>S.Bahri</span><span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--cream-mute)", letterSpacing: "0.15em", textTransform: "uppercase" }}>GTM Consulting</span></div>
      ...
      <section id="about" style={{ padding: "120px 64px", maxWidth: 1560, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 64 }}>
          <div>
            ...
            <h2 style={{ fontFamily: "var(--serif)", fontSize: 40, fontWeight: 400, fontStyle: "italic", color: "var(--cream)", lineHeight: 1.15, marginBottom: 24 }}>The Consultant</h2>
      ...
      <footer style={{ padding: "60px 64px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontFamily: "var(--serif)", fontSize: 18, fontStyle: "italic", color: "var(--cream-mute)" }}>S.Bahri</span>
        <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--cream-mute)", letterSpacing: "0.1em" }}>&copy; {new Date().getFullYear()} / GTM Consulting</span>
      </footer>
    </div>
  );
};
...
// ============================================================
const DashboardPage=({data,user})=>{const my=data.tasks.filter(t=>t.owner===user.id);const stats=[{label:"Active Projects",val:data.projects.filter(p=>p.status==="active").length,c:"var(--amber)"},{label:"Open Tasks",val:my.filter(t=>t.status!=="completed").length,c:"var(--sky)"},{label:"Completed",val:my.filter(t=>t.status==="completed").length,c:"var(--success)"},{label:"Clients",val:data.clients.filter(c=>c.status==="active").length,c:"var(--violet)"}];const recent=[...data.activityLog].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,8);const overdue=my.filter(t=>t.status!=="completed"&&t.dueDate&&new Date(t.dueDate)<new Date());return(<div><div style={{marginBottom:40}}><span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--cream-mute)",letterSpacing:"0.2em",textTransform:"uppercase"}}>Dashboard</span><h1 style={{fontFamily:"var(--serif)",fontSize:36,fontWeight:400,fontStyle:"italic",color:"var(--cream)",marginTop:8}}>Welcome, {user.name.split(" ")[0]}</h1></div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,background:"var(--border)",borderRadius:12,overflow:"hidden",marginBottom:40}}>{stats.map((s,i)=>(<div key={i} style={{padding:"28px 24px",background:"var(--ink-2)"}}><div style={{fontFamily:"var(--serif)",fontSize:40,fontWeight:400,color:s.c,fontStyle:"italic"}}>{s.val}</div><div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--cream-mute)",letterSpacing:"0.1em",textTransform:"uppercase",marginTop:8}}>{s.label}</div></div>))}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}><div style={{padding:28,background:"var(--ink-2)",borderRadius:12,border:"1px solid var(--border)"}}><h3 style={{fontFamily:"var(--serif)",fontSize:20,fontStyle:"italic",color:"var(--cream)",marginBottom:20}}>Recent Activity</h3>{recent.map(a=>(<div key={a.id} style={{padding:"10px 0",borderBottom:"1px solid var(--border)",display:"flex",gap:14,alignItems:"flex-start"}}><span style={{width:5,height:5,borderRadius:"50%",background:"var(--amber)",marginTop:7,flexShrink:0}}/><div><p style={{fontSize:13,color:"var(--cream-dim)",lineHeight:1.5,margin:0}}>{a.details}</p><span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--cream-mute)"}}>{new Date(a.timestamp).toLocaleDateString()}</span></div></div>))}</div><div style={{padding:28,background:"var(--ink-2)",borderRadius:12,border:"1px solid var(--border)"}}><h3 style={{fontFamily:"var(--serif)",fontSize:20,fontStyle:"italic",color:overdue.length?"var(--danger)":"var(--cream)",marginBottom:20}}>{overdue.length?`${overdue.length} Overdue`:"Upcoming Tasks"}</h3>{(overdue.length?overdue:my.filter(t=>t.status!=="completed").slice(0,6)).map(t=>{const p=data.projects.find(p=>p.id===t.projectId);return(<div key={t.id} style={{padding:"10px 0",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><p style={{margin:0,fontSize:13,color:"var(--cream-dim)"}}>{t.title}</p><span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--cream-mute)"}}>{p?.name}</span></div><Badge status={t.status}/></div>);})}{my.filter(t=>t.status!=="completed").length===0&&<p style={{color:"var(--cream-mute)",fontSize:13,fontStyle:"italic"}}>All clear</p>}</div></div></div>);};

const ClientsPage=({data,dispatch,user})=>{const[modal,setModal]=useState(false);const[editing,setEditing]=useState(null);const[form,setForm]=useState({name:"",company:"",email:"",phone:"",industry:"",notes:"",status:"active"});const[q,setQ]=useState("");const openNew=()=>{setForm({name:"",company:"",email:"",phone:"",industry:"",notes:"",status:"active"});setEditing(null);setModal(true);};const openEdit=(c)=>{setForm({...c});setEditing(c.id);setModal(true);};const save=()=>{editing?dispatch({type:"UPDATE_CLIENT",payload:{...form,id:editing}}):dispatch({type:"ADD_CLIENT",payload:{...form,createdBy:user.id},userId:user.id});setModal(false);};const filtered=data.clients.filter(c=>!q||`${c.name} ${c.company}`.toLowerCase().includes(q.toLowerCase()));return(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:40}}><div><span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--cream-mute)",letterSpacing:"0.2em",textTransform:"uppercase"}}>Relationships</span><h1 style={{fontFamily:"var(--serif)",fontSize:36,fontWeight:400,fontStyle:"italic",color:"var(--cream)",marginTop:8}}>Clients</h1></div>{user.role!=="client"&&<Btn icon="plus" onClick={openNew}>New Client</Btn>}</div><div style={{marginBottom:24,position:"relative"}}><span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"var(--cream-mute)"}}><Icon name="search" size={15}/></span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search..." style={{width:"100%",padding:"11px 14px 11px 40px",background:"var(--ink-2)",border:"1px solid var(--border)",borderRadius:8,color:"var(--cream)",fontSize:14,fontFamily:"var(--sans)"}}/></div>{filtered.length===0?<Empty icon="users" title="No clients" action="Add Client" onAction={openNew}/>:(<div style={{display:"grid",gap:1,background:"var(--border)",borderRadius:12,overflow:"hidden"}}>{filtered.map(c=>{const n=data.projects.filter(p=>p.clientId===c.id).length;return(<div key={c.id} onClick={()=>openEdit(c)} style={{padding:"20px 24px",background:"var(--ink-2)",display:"grid",gridTemplateColumns:"48px 1fr auto",gap:16,alignItems:"center",cursor:"pointer",transition:"background .2s"}} onMouseEnter={e=>e.currentTarget.style.background="var(--ink-3)"} onMouseLeave={e=>e.currentTarget.style.background="var(--ink-2)"}><div style={{width:48,height:48,borderRadius:12,background:"var(--ink)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--serif)",fontSize:18,fontStyle:"italic",color:"var(--amber)"}}>{c.name.split(" ").map(w=>w[0]).join("").slice(0,2)}</div><div><p style={{margin:0,fontSize:15,color:"var(--cream)",fontWeight:500}}>{c.name}</p><span style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--cream-mute)"}}>{c.company} / {n} project{n!==1?"s":""}</span></div><Badge status={c.status}/></div>);})}</div>)}<Modal open={modal} onClose={()=>setModal(false)} title={editing?"Edit Client":"New Client"}><Field label="Name" value={form.name} onChange={v=>setForm({...form,name:v})}/><Field label="Company" value={form.company} onChange={v=>setForm({...form,company:v})}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Field label="Email" value={form.email} onChange={v=>setForm({...form,email:v})}/><Field label="Phone" value={form.phone} onChange={v=>setForm({...form,phone:v})}/></div><Field label="Notes" value={form.notes} onChange={v=>setForm({...form,notes:v})} type="textarea"/><div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>{editing&&<Btn v="danger" onClick={()=>{dispatch({type:"DELETE_CLIENT",payload:editing});setModal(false);}}>Delete</Btn>}<Btn v="secondary" onClick={()=>setModal(false)}>Cancel</Btn><Btn onClick={save} disabled={!form.name}>{editing?"Save":"Create"}</Btn></div></Modal></div>);};
